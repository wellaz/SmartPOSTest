        ...
        DialogUtils.showViewDialog(this, view, getString(R.string.input_byte), null,
                getString(R.string.button_ok), getString(R.string.btn_cancel), new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                      final  String input = view.getText().toString().trim();
//                        if (input.length() > 2) {
...
//                                write();
                                //writeProtected();
                                // hex string,  16 digits, 8 bytes

                                String s = input;
                                byte[] writeData = StringUtils.convertHexToBytes(s);
                                int ret = mSLE4442Card.writeData((byte)10, (byte)writeData.length, writeData);
                                Log.e(TAG, "writeData: " + StringUtils.convertBytesToHex(writeData) + "\t" + ret);
                                showLog("writeData: " + StringUtils.convertBytesToHex(writeData) + "\t" + ret);
